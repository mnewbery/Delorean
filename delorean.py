#!/usr/bin/env python3
# NTP MitM Tool
#original code copied from  Jose Selvi - jselvi[a.t]pentester[d0.t]es - http://www.pentester.es
# Greetings for Python3 port to Tristan Rice - https://github.com/d4l3k
# Version 1.4 - 06/Dec/2020
# This version Matthew Newbery 25th February 2025
# Added code for windows to check if the current user is windows local admin while also being compatible with Linux
...
class NTProxy(threading.Thread):
    ...
    def extract(self, data):
        ...
        # Return useful info for response
        return info
...
# Check if root
try:
    # The above code is checking if the `options.port` value is less than or equal to 1024 and if the
    # effective user ID (euid) returned by `os.geteuid()` is not equal to 0. This condition is
    # commonly used to check if the program is trying to bind to a privileged port (port numbers less
    # than or equal to 1024) without having root privileges (euid not equal to 0).
    # geteuid was valid in Python 2.6 and only on Unix variants
    if options.port <= 1024: 
        if os.name != 'nt' and callable(os.geteuid) and os.geteuid() != 0:
            sys.exit("Delorean must be run as root when binding ports under 1024")
        elif os.name == 'nt' and  callable(ctypes.windll.shell32.IsUserAnAdmin) and ctypes.windll.shell32.IsUserAnAdmin() == 0:
            sys.exit("Delorean must be run as root when binding ports under 1024")
...
except OSError: #OSError: [WinError 10048] Only one usage of each socket address (protocol/network address/port) is normally permitted
    sys.exit("Something else is already bound to this port. Shut down any peer time server, NTP client or server before trying again. A full list of ")
